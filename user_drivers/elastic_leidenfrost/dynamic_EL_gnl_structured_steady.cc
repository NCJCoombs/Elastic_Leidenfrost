...
namespace GlobalSimSettings
{
  ...
  double Newmark_beta1 = 0.5;
  double Newmark_beta2 = 0.5;

  /// Newmark parameters at start of simulation
  double Newmark_beta1_start = 0.6;
  double Newmark_beta2_start = 0.605;

  /// Continue simulation once steady state attained?
  unsigned continue_from_steady = 1;
  ...
  unsigned min_refinement_level = 4;
  ...
  // Number of computational timesteps per bulk doc: Large enough that bulk docs
  // are suppressed
  unsigned steps_per_bulk_doc = 100000000;

  // Number of steps per adapt
  unsigned steps_per_adapt = 1;
...
public:
  ...
  VariableNewmark()
  {
    ...
    this->Beta1 = GlobalSimSettings::Newmark_beta1_start;
    this->Beta2 = GlobalSimSettings::Newmark_beta2_start;
  }
...
public:
  ...
  void toggle_bulk_mesh_refinement()
  {
    // Get the min height
    double h_min = get_min_height();
    ...
    // If h_min small enough, use non-dissipative Newmark params
    // if (h_min < 0.005)
    // {
    //   TIMESTEPPER* time_step_pt =
    //     dynamic_cast<TIMESTEPPER*>(this->time_stepper_pt());
    //   time_step_pt->set_newmark_parameters(GlobalSimSettings::Newmark_beta1,
    //                                        GlobalSimSettings::Newmark_beta2);
    //   std::cout << "THE NEWMARK PARAMETERS ARE: "
    //             << GlobalSimSettings::Newmark_beta1 << " "
    //             << GlobalSimSettings::Newmark_beta2 << std::endl;
    // }
  ...
  double get_min_height()
  {
    ...
    {
      double h_tmp = Bulk_mesh_pt->node_pt(n)->x(1);
      if (h_tmp < h_min)
      {
        h_min = h_tmp;
      }
  ...
  complete_problem_setup()
{
  ...
  for (unsigned e = 0; e < n_element_bulk; e++)
  {
    ...
    el_pt->eta_mass_pt() = &GlobalPhysicalVariables::eta_M;
  ...
  compute_contact_radius_and_avg_height(double& l, double& h_avg)
{
  ...
  // sampled)
  unsigned n_interior = 10;

  // Tolerance for variation in the normal
  double eps_normal = 0.01;
  ...
  unsteady_run()
{
  ...
  // Turn gravity off initially and set mass damping param to zero
  double g_tmp = GlobalPhysicalVariables::g_non_dim;
  GlobalPhysicalVariables::g_non_dim = 0.0;
  double eta_M_tmp = GlobalPhysicalVariables::eta_M;
  GlobalPhysicalVariables::eta_M = 0.0;
  double DTSF_min_decrease_tmp = DTSF_min_decrease;
  DTSF_min_decrease = 1.0e-8;
  ...
  // Cache the initial kinetic energy
  double E_g, E_k, E_e, p_surf;
  compute_total_energy(E_g, E_k, E_e, p_surf);
  double E_k_intial = E_k;
  ...
  while (success)
  {
    ...
    // If we're far enough into the simulation, turn gravity back on as well as
    // mass damping
    if (time_pt()->time() > (GlobalPhysicalVariables::h_initial - 1.0) /
                              GlobalPhysicalVariables::W_non_dim)
    {
      std::cout << "GRAVITY HAS BEEN RESET" << std::endl;
      GlobalPhysicalVariables::g_non_dim = g_tmp;
      GlobalPhysicalVariables::eta_M = eta_M_tmp;
    }

    // If enough energy has been damped out: remove damping, restore Newmark
    // parameters, turn off adaptation, impose min dt decerease
    compute_total_energy(E_g, E_k, E_e, p_surf);
    if (E_k / E_k_intial < 1.0e-6)
    {
      // Are we still going
      if (GlobalSimSettings::continue_from_steady)
      {
        std::cout << "NEWMARK PARAMETERS SET TO 0.5 AND DAMPING REMOVED"
                  << std::endl;
        GlobalPhysicalVariables::eta_M = 0.0;
        TIMESTEPPER* time_step_pt =
          dynamic_cast<TIMESTEPPER*>(this->time_stepper_pt());
        time_step_pt->set_newmark_parameters(GlobalSimSettings::Newmark_beta1,
                                             GlobalSimSettings::Newmark_beta2);
        adapt_flag = false;
        DTSF_min_decrease = DTSF_min_decrease_tmp;
        GlobalSimSettings::epsilon_t = 1.0e-5;
        GlobalSimSettings::steps_per_bulk_doc = 1;
      }
      else // Otherwise break out
      {
        oomph_info << "STEADY STATE REACHED. BREAKING OUT." << std::endl;
        break;
      }
    ...
    {
      // Limit the number of steps after the steady state has been reached
      n++;
    }

    if (n == 10000)
    {
      std::cout << "MAXIMUM NUMBER OF STEPS REACHED: BREAKING OUT" << std::endl;
      break;
    }
...
int main(int argc, char* argv[])
{
  ...
  CommandLineArgs::specify_command_line_flag(
    "--eta_mass", &GlobalPhysicalVariables::eta_M, "Mass damping parameter");
  ...
  CommandLineArgs::specify_command_line_flag(
    "--continue_from_steady",
    &GlobalSimSettings::continue_from_steady,
    "Running past steady state");

  CommandLineArgs::parse_and_assign();
  CommandLineArgs::doc_specified_flags();

  // If the given start height is less than the sim start height, set them equal
